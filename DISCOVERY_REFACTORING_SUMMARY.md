...
1. **`acb/services/discovery.py`**

   - `import_service()`: 45 → **2** (96% reduction)
   - Lines removed: 72
   - Lines added: 10

1. **`acb/testing/discovery.py`**
...
1. **Function Extraction**: Broke down monolithic function into focused helpers
1. **Type-Based Dispatching**: Simple isinstance checks instead of nested conditionals
1. **Simplified Context Detection**: Cleaner file reading with single try/except
1. **Protocol-Based Configuration**: Dependency injection for flexible behavior
...
1. ✅ **Shared helper utilities** - Complete `discovery_common.py` module created
1. ✅ **Complexity reduction metrics** - Detailed before/after analysis provided
1. ✅ **Test compatibility verification** - Import and basic functionality verified

## Next Steps (Optional)

1. Run full test suite to verify all edge cases
1. Update CHANGELOG.md with refactoring details
1. Consider applying similar pattern to other discovery systems in ACB
1. Add unit tests specifically for `discovery_common.py` helpers
