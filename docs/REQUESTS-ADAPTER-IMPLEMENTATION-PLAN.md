...
2. ✅ **RFC 9111 Compliance**: 80% HTTP caching specification compliance
3. ✅ **ACB Pattern Compliance**: Fix CleanupMixin, method naming, context managers
4. ✅ **Serialization Fixed**: msgspec-based serialization for Redis compatibility
5. ✅ **GraphQL Support**: POST body-based cache keys per README documentation
...
2. **Add Universal Cache Section**: Document how caching works
3. **Update Architecture Diagram**: Show UniversalHTTPCache
4. **Add Context Manager Examples**: Show async with usage
5. **Update GraphQL Section**: Explain POST body caching implementation
...
2. **Breaking Change for Hishel Users** (Risk: Low)
...
2. **CleanupMixin Addition** - Standard ACB pattern, matches 13 other adapters
3. **Method Renaming** - Internal only, no public API change
