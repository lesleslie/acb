...
class CustomFeatureStoreAdapter(BaseFeatureStoreAdapter):
    ...
    async def get_online_features(
        ...
        try:
                ...
                for feature_name in request.feature_names:
                    ...
                    if value is not None:
                        ...
                        if (
                            ...
                            < self.custom_settings.in_memory_cache_size
                        ):
            ...
            cache_hit_ratio = (
                len([v for v in feature_vectors if v.features]) / len(feature_vectors)
                if feature_vectors
                else 0.0
            )
    ...
    async def get_historical_features(
        ...
        try:
            ...
            if self.custom_settings.storage_type == "sqlite" and self._db_connection:
                ...
                for feature_name in feature_names:
                    feature_file = (
                        self._storage_path / "features" / f"{feature_name}.parquet"
                    )
            ...
            if self.custom_settings.storage_type == "sqlite" and self._db_connection:
                ...
                for column in df.columns:
                    if column not in ["entity_id", "timestamp", "event_time"]:
                        try:
                            feature_file = (
                                self._storage_path / "features" / f"{column}.parquet"
                            )
