...
class KServeSettings(MLModelSettings):
    ...
    model_format: str = Field(
        default="tensorflow", description="Model format (tensorflow, pytorch, sklearn, etc.)"
    )
    ...
    scale_to_zero: bool = Field(
        default=False, description="Enable scale-to-zero"
    )
    ...
    enable_canary: bool = Field(
        default=False, description="Enable canary deployment"
    )
    ...
    log_url: Optional[str] = Field(default=None, description="Custom log URL")
    enable_explainer: bool = Field(
        default=False, description="Enable model explainer"
    )
...
class KServeAdapter(BaseMLModelAdapter):
    ...
    def __init__(self, settings: Optional[KServeSettings] = None) -> None:
    ...
    async def _create_client(self) -> Dict[str, Any]:
        ...
        connector = aiohttp.TCPConnector(limit=self.kserve_settings.connection_pool_size)
    ...
    def _get_inference_url(self, model_name: str, version: Optional[str] = None) -> str:
        ...
        service_name = f"{model_name}-predictor" if not version else f"{model_name}-{version}-predictor"
    ...
    def _create_inference_service_spec(self, model_name: str, model_path: str, version: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self.kserve_settings.runtime_version:
            spec["spec"]["predictor"][self.kserve_settings.model_format]["runtimeVersion"] = self.kserve_settings.runtime_version
        
        # Add protocol version
        if self.kserve_settings.protocol_version == "v2":
            spec["spec"]["predictor"][self.kserve_settings.model_format]["protocolVersion"] = "v2"
        
        # Add GPU resources if specified
        if self.kserve_settings.gpu_count > 0:
            gpu_resource = f"nvidia.com/gpu"
            if self.kserve_settings.gpu_type:
                gpu_resource = f"nvidia.com/{self.kserve_settings.gpu_type}"
            
            spec["spec"]["predictor"][self.kserve_settings.model_format]["resources"]["limits"][gpu_resource] = str(self.kserve_settings.gpu_count)
            spec["spec"]["predictor"][self.kserve_settings.model_format]["resources"]["requests"][gpu_resource] = str(self.kserve_settings.gpu_count)
        ...
        if self.kserve_settings.enable_canary:
            spec["spec"]["canaryTrafficPercent"] = self.kserve_settings.canary_traffic_percent
        ...
        if self.kserve_settings.enable_explainer and self.kserve_settings.explainer_type:
            spec["spec"]["explainer"] = {
            ...
            if self.kserve_settings.protocol_version == "v2":
                ...
                payload = {
                    "inputs": [
                        {
                            "name": key,
                            "shape": [1] if not isinstance(value, list) else [len(value)],
                ...
                payload = {
                    "instances": [request.inputs] if isinstance(request.inputs, dict) else request.inputs
                }
            ...
            if self.kserve_settings.protocol_version == "v2":
                ...
                payload = {
                    "inputs": [
                        {
                            "name": key,
                            "shape": [len(values)] + ([1] if not isinstance(values[0], list) else [len(values[0])]),
                            "datatype": "FP32",
                            "data": values,
                        }
                    ...
                    if outputs:
                        ...
                        for i in range(batch_size):
                            ...
                            for output in outputs:
                                ...
                                if isinstance(output_data, list) and len(output_data) > i:
    ...
    async def list_models(self) -> List[ModelInfo]:
        ...
        try:
            ...
            for item in response.get("items", []):
                ...
                for framework_key in ["tensorflow", "pytorch", "sklearn", "xgboost", "triton"]:
                ...
                models.append(
                    ModelInfo(
                        ...
                        framework=framework,
                        description=f"KServe InferenceService",
                        created_at=metadata.get("creationTimestamp"),
                        metadata={
                            "platform": "kserve",
                            "namespace": self.kserve_settings.namespace,
                            "storage_uri": storage_uri,
                            "protocol_version": spec.get("predictor", {}).get(framework, {}).get("protocolVersion", "v1"),
    ...
    async def load_model(
        ...
        try:
            ...
            # Create InferenceService specification
            inference_service = self._create_inference_service_spec(model_name, model_path, version)
