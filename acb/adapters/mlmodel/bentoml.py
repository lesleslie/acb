...
class BentoMLSettings(MLModelSettings):
    ...
    bento_store_uri: Optional[str] = Field(
        default=None, description="BentoML store URI"
    )
    model_store_uri: Optional[str] = Field(
        default=None, description="Model store URI"
    )
    
    # Service settings
    service_name: Optional[str] = Field(
        default=None, description="BentoML service name"
    )
    bento_tag: Optional[str] = Field(
        default=None, description="Bento tag (name:version)"
    )
    ...
    base_image: Optional[str] = Field(
        default=None, description="Base container image"
    )
...
class BentoMLAdapter(BaseMLModelAdapter):
    ...
    def __init__(self, settings: Optional[BentoMLSettings] = None) -> None:
    ...
    async def _create_client(self) -> Dict[str, Any]:
        ...
        if not BENTOML_AVAILABLE:
            raise RuntimeError("BentoML not available. Install with: pip install bentoml")
        
        # Configure BentoML stores
        if self.bentoml_settings.bento_store_uri:
            self._bento_store = bentoml.bento_store.BentoStore(self.bentoml_settings.bento_store_uri)
        else:
            self._bento_store = bentoml.bento_store.BentoStore()
        
        if self.bentoml_settings.model_store_uri:
            self._model_store = bentoml.models.ModelStore(self.bentoml_settings.model_store_uri)
        else:
            self._model_store = bentoml.models.ModelStore()
        
        # Create HTTP session for API calls
        connector = aiohttp.TCPConnector(limit=self.bentoml_settings.connection_pool_size)
    ...
    def _get_api_url(self, endpoint: str) -> str:
        ...
        base_url = f"{protocol}://{self.bentoml_settings.host}:{self.bentoml_settings.port}"
            ...
            for model in model_store.list():
                ...
                models.append(
                    ModelInfo(
                        ...
                        created_at=pd.Timestamp.fromtimestamp(model_info.creation_time).isoformat(),
                        metadata={
                            "platform": "bentoml",
                            "tag": str(model.tag),
                        ...
                        if response.status == 200:
                            ...
                            for line in metrics_text.split('\n'):
                                if 'bentoml_service_request_total' in line:
                                    bentoml_metrics["total_requests"] = float(line.split()[-1])
                                elif 'bentoml_service_request_duration_seconds' in line and 'quantile="0.95"' in line:
                                    bentoml_metrics["p95_latency_ms"] = float(line.split()[-1]) * 1000
