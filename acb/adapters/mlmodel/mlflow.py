...
class MLflowSettings(MLModelSettings):
    ...
    model_registry_uri: Optional[str] = Field(
        default=None, description="MLflow model registry URI (defaults to tracking_uri)"
    )
    artifact_uri: Optional[str] = Field(
        default=None, description="MLflow artifact store URI"
    )
    
    # Model serving settings
    serving_uri: Optional[str] = Field(
        default=None, description="MLflow model serving endpoint"
    )
    serving_port: int = Field(default=5001, description="MLflow model serving port")
    
    # Model registry settings
    registered_model_name: Optional[str] = Field(
        default=None, description="Default registered model name"
    )
    model_stage: str = Field(
        default="Production", description="Model stage (Production, Staging, Archived)"
    )
    model_version: Optional[str] = Field(
        default=None, description="Specific model version"
    )
    ...
    enable_model_serving: bool = Field(
        default=True, description="Enable MLflow model serving"
    )
    ...
    enable_model_versioning: bool = Field(
        default=True, description="Enable automatic model versioning"
    )
    enable_model_signature: bool = Field(
        default=True, description="Enable model signature validation"
    )
    enable_input_example: bool = Field(
        default=True, description="Store input examples with models"
    )
    
    # Authentication
    mlflow_username: Optional[str] = Field(
        default=None, description="MLflow username"
    )
    mlflow_password: Optional[str] = Field(
        default=None, description="MLflow password"
    )
...
class MLflowAdapter(BaseMLModelAdapter):
    ...
    def __init__(self, settings: Optional[MLflowSettings] = None) -> None:
    ...
    async def _create_client(self) -> Dict[str, Any]:
        ...
        if not MLFLOW_AVAILABLE:
            raise RuntimeError("MLflow not available. Install with: pip install mlflow")
        
        # Set MLflow tracking URI
        mlflow.set_tracking_uri(self.mlflow_settings.tracking_uri)
        
        # Create MLflow client
        registry_uri = self.mlflow_settings.model_registry_uri or self.mlflow_settings.tracking_uri
        ...
        connector = aiohttp.TCPConnector(limit=self.mlflow_settings.connection_pool_size)
        ...
        if self.mlflow_settings.mlflow_username and self.mlflow_settings.mlflow_password:
            import base64
    ...
    def _get_serving_url(self, model_name: Optional[str] = None, version: Optional[str] = None) -> str:
    ...
    def _get_model_uri(self, model_name: str, version: Optional[str] = None, stage: Optional[str] = None) -> str:
    ...
    async def predict(self, request: ModelPredictionRequest) -> ModelPredictionResponse:
        ...
        try:
            ...
            model_uri = self._get_model_uri(request.model_name, request.model_version)
            serving_url = self._get_serving_url(request.model_name, request.model_version)
            ...
            async with http_session.post(serving_url, json=payload) as response:
                if response.status != 200:
                    ...
                    raise RuntimeError(f"MLflow serving error: {error_text}")
    ...
    async def batch_predict(
        ...
        try:
            ...
            async with http_session.post(serving_url, json=payload) as response:
                if response.status != 200:
                    error_text = await response.text()
                    raise RuntimeError(f"MLflow batch serving error: {error_text}")
            ...
            raise RuntimeError(f"MLflow batch prediction failed: {e}")
    ...
    async def list_models(self) -> List[ModelInfo]:
        ...
        try:
            ...
            for registered_model in mlflow_client.search_registered_models():
                ...
                for model_version in mlflow_client.search_model_versions(f"name='{model_name}'"):
                    ...
                    models.append(
                        ModelInfo(
                            ...
                            framework="mlflow",
                            created_at=pd.Timestamp.fromtimestamp(model_version.creation_timestamp / 1000).isoformat(),
                            updated_at=pd.Timestamp.fromtimestamp(model_version.last_updated_timestamp / 1000).isoformat(),
                            metrics=metrics,
                            metadata={
                                "platform": "mlflow",
                                "run_id": run_id,
            ...
            raise RuntimeError(f"Failed to list MLflow models: {e}")
        ...
        try:
            ...
            if version:
                ...
                versions = mlflow_client.get_latest_versions(
                    model_name, stages=[self.mlflow_settings.model_stage]
                )
                if not versions:
                    raise RuntimeError(f"No model found in stage {self.mlflow_settings.model_stage}")
            ...
            try:
                ...
                input_schema = model.metadata.signature.inputs.to_dict() if model.metadata.signature else None
                output_schema = model.metadata.signature.outputs.to_dict() if model.metadata.signature else None
            except:
                input_schema = None
                output_schema = None
            ...
            raise RuntimeError(f"Failed to get MLflow model info: {e}")
    ...
    async def get_model_health(
        ...
        try:
            ...
            health_status = "healthy" if model_info.status == "production" else "unhealthy"
    ...
    async def load_model(
        ...
        try:
            ...
            raise RuntimeError(f"MLflow model registration failed: {e}")
    ...
    async def scale_model(
        ...
        raise NotImplementedError("Model scaling should be handled by deployment platform")
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            mlflow_metrics = {
                "registered_models_count": len(mlflow_client.search_registered_models()),
                "tracking_uri": self.mlflow_settings.tracking_uri,
                "model_registry_uri": self.mlflow_settings.model_registry_uri or self.mlflow_settings.tracking_uri,
            }
            
            # Get experiment metrics if experiment is configured
            if self.mlflow_settings.experiment_name:
                try:
                    experiment = mlflow_client.get_experiment_by_name(self.mlflow_settings.experiment_name)
                    if experiment:
