...
class WorkflowOrchestrator:
        ...
        try:
            ...
            for i, step in enumerate(steps):
                ...
                if component_type == "action":
                    result = await self._execute_action_step(component_name, action, parameters)
                elif component_type == "adapter":
                    result = await self._execute_adapter_step(component_name, action, parameters)
                else:
    ...
    async def _execute_action_step(self, action_category: str, action_name: str, parameters: Dict[str, Any]) -> Any:
    ...
    async def _execute_adapter_step(self, adapter_name: str, method_name: str, parameters: Dict[str, Any]) -> Any:
        ...
        if not method:
            raise ValueError(f"Method '{method_name}' not found in adapter '{adapter_name}'")
    ...
    async def start_background_workflow(self, workflow_id: str, steps: List[Dict[str, Any]]) -> None:
