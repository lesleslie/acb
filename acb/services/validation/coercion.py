...
class TypeCoercer:
    ...
    async def coerce_to_type(
        ...
        if self.strategy == CoercionStrategy.STRICT:
            result.add_error(f"Type mismatch: expected {target_type.__name__}, got {type(data).__name__}")
            result.validation_time_ms = (time.perf_counter() - start_time) * 1000
            return result
        ...
        try:
            ...
            result.add_warning(f"Value coerced from {type(data).__name__} to {target_type.__name__}")
    ...
    async def _coerce_to_int(self, data: t.Any) -> int:
        ...
        if isinstance(data, bool):
                ...
                try:
                    ...
                    if '.' in data:
                        ...
                        if float_val.is_integer():
                            ...
                            raise ValueError(f"Cannot convert decimal string '{data}' to int")
                    return int(data)
                except ValueError:
            ...
            if data % 1 == 0:
                ...
                raise ValueError(f"Cannot convert Decimal {data} to int without precision loss")
        else:
            return int(data)
    ...
    async def transform_case(
        ...
        try:
            if case_type == "lower":
                ...
                result.value = re.sub(r'[A-Z]', r'_\g<0>', data).lower().lstrip('_')
            elif case_type == "camel":
                # Convert to camelCase
                components = data.split('_')
                result.value = components[0] + ''.join(x.title() for x in components[1:])
...
class RangeValidator:
    ...
    async def validate_numeric_range(
        self,
        data: t.Union[int, float, Decimal],
        min_value: t.Union[int, float, Decimal] | None = None,
        max_value: t.Union[int, float, Decimal] | None = None
    ) -> ValidationResult:
