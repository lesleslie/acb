...
class UnitOfWorkError(RepositoryError):
    ...
    def __init__(self, message: str, transaction_id: str | None = None, state: UnitOfWorkState | None = None):
...
class UnitOfWork(CleanupMixin):
    ...
    def __init__(self, isolation_level: str | None = None, timeout: float | None = None):
        super().__init__()
...
class UnitOfWorkManager(CleanupMixin):
    ...
    async def transaction(
        ...
        isolation_level: str | None = None,
        timeout: float | None = None
    ) -> t.AsyncGenerator[UnitOfWork, None]:
    ...
    async def get_transaction_history(self, limit: int = 100) -> List[UnitOfWorkMetrics]:
    ...
    async def get_transaction_stats(self) -> Dict[str, Any]:
        ...
        success_count = sum(
            1 for metrics in recent_transactions
            if metrics.state == UnitOfWorkState.COMMITTED
        )
        success_rate = success_count / len(recent_transactions) if recent_transactions else 0.0

        # Calculate average duration
        durations = [
            metrics.duration for metrics in recent_transactions
    ...
    async def _complete_transaction(self, uow: UnitOfWork) -> None:
        ...
        if len(self._completed_transactions) > self._max_completed_history:
            self._completed_transactions = self._completed_transactions[-self._max_completed_history:]
