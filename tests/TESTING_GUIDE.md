...
2. Make sure tests are isolated (dependency container is reset automatically)
3. Use the shared `mock_config` fixture instead of creating your own
4. Add `@pytest.mark.quick` to tests that run fast (under 1 second)
5. Mark external service tests with `@pytest.mark.external`
...
2. Use `--ff` (failed first) to run failed tests first
3. Run specific test files directly for faster feedback during development
4. Use category-based execution for CI/CD pipeline optimization
