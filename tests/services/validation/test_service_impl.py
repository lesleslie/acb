...
class _Schema(ValidationSchema):
    def __init__(self, name: str, *, valid: bool = True, value: Any | None = None, raise_exc: bool = False) -> None:
        super().__init__(name)
        self._valid = valid
        self._value = value
    ...
    async def validate(self, data: Any, field_name: str | None = None) -> ValidationResult:  # type: ignore[override]
        ...
        res = ValidationResult(field_name=field_name, value=val, original_value=data, is_valid=self._valid)
        if not self._valid:
            res.add_error("bad")
        return res
...
async def test_validate_basic_sanitization(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("acb.services.validation.service.time.perf_counter", lambda: 0.0)
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_validate_with_schema_and_sanitization(monkeypatch: pytest.MonkeyPatch) -> None:
    svc = ValidationService()
    svc.logger = MagicMock()
    cfg = ValidationConfig(enable_sanitization=True, enable_xss_protection=True)
...
async def test_validate_with_schema_exception(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    res = await svc.validate("x", schema=schema, config=ValidationConfig(), field_name="f")
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_validate_many_labels_and_results(monkeypatch: pytest.MonkeyPatch) -> None:
    svc = ValidationService()
    svc.logger = MagicMock()
