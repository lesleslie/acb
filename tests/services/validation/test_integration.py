...
class TestValidationSystemIntegration:
    ...
    async def test_complete_user_registration_flow(
        ...
        builder.add_string("username", min_length=3, max_length=20, pattern=r"^[a-zA-Z0-9_]+$")
        ...
        for field, value in user_data.items():
            ...
            if schema:
                result = await validation_service.validate(value, schema, field_name=field)
    ...
    async def test_api_input_validation_with_sanitization(
        ...
        @validate_input({
            ...
            "content": StringValidationSchema("content", min_length=10, max_length=1000)
        })
    ...
    async def test_schema_compilation_performance(
        ...
        first_time = validation_service.get_metrics().total_validation_time_ms - start_time
        ...
        start_time = validation_service.get_metrics().total_validation_time_ms
        result2 = await validation_service.validate("another@example.com", complex_schema)
        second_time = validation_service.get_metrics().total_validation_time_ms - start_time
