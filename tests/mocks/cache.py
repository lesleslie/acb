...
class MockCacheBase(CacheBase):
    """Base mock cache implementation with common functionality."""
    ...
    def namespace(self, namespace: str) -> "MockCacheBase":
        # Create new instance of the same type
        namespaced_cache = self.__class__()
        namespaced_cache._namespace = f"{self._namespace}{namespace}:"
        # Copy any additional attributes from the current instance
        for attr_name, attr_value in self.__dict__.items():
            if not attr_name.startswith("_"):
                setattr(namespaced_cache, attr_name, attr_value)
...
class MockMemoryCache(MockCacheBase):
    """Mock memory cache implementation."""

    pass


class MockRedisCache(MockCacheBase):
    """Mock Redis cache implementation with Redis-specific attributes."""
